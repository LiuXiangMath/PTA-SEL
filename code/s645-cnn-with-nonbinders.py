...
def main(typ,N):
    ...
    #res1 = np.zeros((645,800))
    #res2 = np.zeros((645,10))
    res3 = np.zeros((645,1))
    final_pcc = []
    for i in range(10):
        print('CV',i)
        ...
        train_feature = np.zeros((645-test_length,feature_shape[1]))
        pre_train_label = []
        ...
        train_feature = train_feature.reshape((645-test_length,grid_number,feature_number))
    ...
    #filename = './s645/27cnn-feature/' + str(N) + '_40_20_feature800_' + typ + '_0.25.npy'
    #np.save(filename,res1)
    
    #filename = './s645/27cnn-feature/' + str(N) + '_40_20_feature10_' + typ + '_0.25.npy'
    #np.save(filename,res2)
    
    filename = './s645/27cnn-feature/' + str(N) + '_40_20_prediction_' + typ + '_0.25.npy'
    np.save(filename,res3)
    #print(typ,np.mean(final_pcc))


def get_combine(N):
    filename = './s645/27cnn-feature/' + N + '_40_20_prediction_mutate2_C_2_h0_0.25.npy'
    d1 = np.load(filename)
    for typ1 in site_typ:
        for typ2 in atom_typ:
            for typ3 in tor_typ:
                ...
                filename = './s645/27cnn-feature/' + N + '_40_20_prediction_' + name + '_0.25.npy'
                d2 = np.load(filename)
                d1 = np.hstack((d1,d2))
    filename = './s645/27cnn-feature/' + N + 'cnn.npy'
    np.save(filename,d1)
