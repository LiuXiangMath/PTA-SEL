...
def get_each_feature_for_stacking(typ,typ2):
    ...
    for i in range(10):
        ...
        train_feature = np.zeros((1131-temp,feature_shape[1])) # 645-27 or 1131 or 645
        pre_train_label = []
        test_c = 0
        train_c = 0
        for j in range(10):
    ...
    final_res = np.zeros((1131,1)) # 645 or 1131
    all_index = []
    for i in range(10):
...
def get_combine(N):
    filename = './skempi/gbt-feature/gbt_' + N + '_mutate2_C_2_h0_aux.npy'
    d1 = np.load(filename)
    for typ1 in site_typ:
        for typ2 in atom_typ:
            for typ3 in tor_typ:
                ...
                filename = './skempi/gbt-feature/gbt_' + N + '_' + name + '_.npy'
                d2 = np.load(filename)
                d1 = np.hstack((d1,d2))
    filename = './skempi/gbt-feature/' + N + 'gbt.npy'
    np.save(filename,d1)
