...
def main(typ,N):
    ...
    for i in range(10):
        ...
        train_feature = np.zeros((645-27-test_length,feature_shape[1]))
        pre_train_label = []
        ...
        train_feature = train_feature.reshape((645-27-test_length,grid_number,feature_number))
    ...
    #filename = './s645/cnn-feature/' + str(N) + '_40_20_feature800_' + typ + '_0.25.npy'
    #np.save(filename,res1)
    
    #filename = './s645/cnn-feature/' + str(N) + '_40_20_feature10_' + typ + '_0.25.npy'
    #np.save(filename,res2)
    
    filename = './s645/cnn-feature/' + str(N) + '_40_20_prediction_' + typ + '_0.25.npy'
    np.save(filename,res3)
    #print(typ,np.mean(final_pcc))


def get_combine(N):
    filename = './s645/cnn-feature/' + N + '_40_20_prediction_mutate2_C_2_h0_0.25.npy'
    d1 = np.load(filename)
    for typ1 in site_typ:
        for typ2 in atom_typ:
            for typ3 in tor_typ:
                ...
                filename = './s645/cnn-feature/' + N + '_40_20_prediction_' + name + '_0.25.npy'
                d2 = np.load(filename)
                d1 = np.hstack((d1,d2))
    filename = './s645/cnn-feature/' + N + 'cnn.npy'
    np.save(filename,d1)
